...
/////////////////////////////////////////////////////////////////////
{
  ...
  if (isIncontact)
  {
    // SMS has to be: LED ON,position 1 on SIM,
     // if position 1 on sim is 04168262667, sms has to be LED ON,0416,
	if (lastLine.indexOf("LED ON") >= 0)
    {
      
	  prendeapaga(0);
    }
      // SMS has to be: LED OFF,position 1 on SIM,
     // if position 1 on SIM is 04168262667, sms has to be LED OFF,0416,
	else if (lastLine.indexOf("LED OFF") >= 0)
    {
      prendeapaga(1);
    }
	//only the position from 1 to 5 on SIM can add new numbers     
    // SMS has to be: ADD,position on sim to be saved,number to save,
    // Example: ADD,8,04168262669,      
    else if (lastLine.indexOf("ADD") >= 0)
    {
      DelAdd(1);
    }
	//only the position from 1 to 5 on SIM can delete numbers    
    // SMS has to be: DEL,position on sim to delete,
    // Example: DEL,8, 
